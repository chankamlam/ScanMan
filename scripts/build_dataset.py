...
原始数据的获取：``scripts/download_data.py --datasets cvefixes`` 可以直接
下载并断点续传 CVEfixes；其余三个源的原始文件需自行按上面「预期字段与布局」
放进 ``data/raw/<源名>/`` 下（本脚本不负责下载）。
...
def prepare_task_data(
    ...
    **classification（多分类）**：只保留"有漏洞 + 带 CWE 编号"的样本
    ...
    -----------------------------------
    CWE 类别之间的样本量差异极大（CVEfixes 上 CWE-79 有 1103 条，
    CWE-1333 只有几十条）。
